...
//Unlike Jasmine, the test framework we're using in this project 
//doesn't come with assertions built-in, so we need to get assertions 
//from another library.
const { expect, assert } = require('chai');

// JSDOM lets us pretend we're in a web browser even through 
//we're running this code in Node. Here, we import JSOM and 
//create fake document and window objects. 
//We assign these to global so they're available to your code, 
//when we run it.
...
// You've made it through Slice 1 – great! 
//If these two tests are passing, you should be able to open 
//the page up in your browser 
//(do `npm run start` and then visit http://localhost:8000) 
//and see some functionality. At the bottom of `script.js`, 
//you'll see some code that attaches your `clickCoffee` function 
//to the coffee emoji you see on screen (this is just a div element). 
//Now, when you click the emoji, you should see the counter update 
//(because of the `updateCoffeeView` function).
...
describe('Slice 2: Unlocking & Rendering Producers', function() {
  ...
  // Inside renderProducers you should probably be calling 
  // *three* functions written previously.
